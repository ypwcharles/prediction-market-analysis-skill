...
def _build_observations(conn: sqlite3.Connection, trigger_row: sqlite3.Row) -> dict[str, object]:
...
def _evaluate_triggers(
    ...
    *,
    ...
    for row in rows:
        trigger = dict(row)
        observations = _build_observations(conn, row)
...
def run_monitor(
    ...
    *,
    ...
    fired_actions, pending_recheck_actions = _evaluate_triggers(conn, now=monitor_now)
