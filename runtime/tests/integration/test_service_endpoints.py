...
def test_status_reports_running_scan_before_new_run_row_exists(tmp_path, monkeypatch):
    app = _build_app(tmp_path, monkeypatch)
    conn = connect_db(tmp_path / ".runtime-data" / "sqlite" / "runtime.sqlite3")
    apply_migrations(conn)
    conn.execute(
    ...
    with app.state.scheduler._state_lock:
        app.state.scheduler._job_state["scan"]["is_running"] = True
        app.state.scheduler._job_state["scan"]["last_started_at"] = "2026-04-24T13:37:47+00:00"
        app.state.scheduler._job_state["scan"]["last_finished_at"] = None
    ...
    assert scan == {
        "id": None,
        "status": "running",
        "started_at": "2026-04-24T13:37:47+00:00",
        "finished_at": None,
        "degraded_reason": None,
        "created_at": "2026-04-24T13:37:47+00:00",
    }
